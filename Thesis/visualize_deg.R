...
bg_ON <- ballgown(dataDir="abundance",samplePattern="ERR",pData=pheno_data)
...
bg_ON_filt<- subset(bg_ON,"rowVars(texpr(bg_ON))>1",genomesubset=TRUE)


#Let's test on transcripts
de_transcripts <- stattest(bg_ON_filt,feature="transcript",covariate="sex",adjustvars=c("population"),getFC=TRUE,meas="FPKM")
...
bg_filt_table=texpr(bg_ON_filt,'all')
...
png('plots/XIST.png')
myplot=plotTranscripts(ballgown::geneIDs(bg_ON)[ballgown::geneNames(bg_ON) == "XIST"], bg_ON, main=c('Gene XIST in sample ERR188234'), sample=c('ERR188234'))
print(myplot)
dev.off()
#DONE
